...
53. **Sunday is a hard-blocked day off — no audits or installations bookable** (2026-07-11): `isSunday(ds)` helper added to `SM_Audit_Dashboard.html`, `SM_Install_Dashboard.html`, and (inline check) `Store_Team_App.html`. Scope is **site-visit dates only** — warehouse `delivery_date`/`#newDeliv`/`#ao-delivery` is deliberately untouched (that's a courier/logistics date, not staff attendance).
    - `SM_Audit_Dashboard.html`: `capFor(aid,ds)` returns `0` for any Sunday before checking `CAPS` overrides — an admin cannot type a nonzero cap for Sunday, it's always ignored. `auditorsView()` greys out and disables the Sunday cap cell (same visual treatment as before-`active_from` cells) with a "Sunday — day off" tooltip. The `#bookDate` picker (shared by the service-creation and reschedule flows, wired through `updateBookBtn()`), `#schSlotDate`, and `#inlineSlotDate` all clear/reject a Sunday selection with toast `"Sundays are a day off — pick another date"`.
    - `SM_Install_Dashboard.html`: standard-mode `#stepDate_${sj.id}` (Step 1 booking) rejects Sunday the same way. Custom/Multi-day mode's `dateRange(from,to)` now **silently skips Sunday** from the generated `dates[]` array — so a multi-day range spanning a Sunday just never schedules that specific day as installer work; the `data-from`/`data-to` inputs show a non-blocking toast if the picked boundary itself is a Sunday, since the range may legitimately span one.
    - `Store_Team_App.html`: `initDateNav()` iterates up to 30 calendar days to collect 14 **non-Sunday** chips (it no longer stops at a flat 14-calendar-day window) — the store team never sees a Sunday option at all. If "today" itself is a Sunday, `SELECTED_DATE` defaults to the next available (Monday) chip, not to today.
    - Do not remove the `capFor`/`dateRange` Sunday short-circuits — they're the actual enforcement; the disabled UI cells are just the visual reflection of it.
